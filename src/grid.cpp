...
double Grid::d1(const std::vector<double>& u, size_t i) const {
    ...
    if(m_rho_uniform) {
        ...
        if(i == 0) {
            return (u[1] - u[0]) / m_d_rho;
        }
        if(i == m_n_rho - 1) {
            return (u[m_n_rho - 1] - u[m_n_rho - 2]) / m_d_rho;
        }
    }
    // for log grid we have a non uniform spacing in rho, but a uniform spacing in t = log(rho), du
    // / drho = du / dt * dt / drho = du / dt 1 / rho
    else {
        ...
        if(i == m_n_rho - 1) {
            du_dt = (u[m_n_rho - 1] - u[m_n_rho - 2]) / dt;
        }
...
double Grid::d2(const std::vector<double>& u, size_t i) const {
    ...
    if(m_rho_uniform) {
        ...
        if(i >= 2 && i <= m_n_rho - 3) {
            return (-u[i + 2] + 16.0 * u[i + 1] - 30.0 * u[i] + 16.0 * u[i - 1] - u[i - 2]) /
                   (12.0 * m_d_rho * m_d_rho);
        }
        // one point from boundary: 2nd order finite difference
        if(i == 1 || i == m_n_rho - 2) {
        ...
        // boundary: 1st order one sided
        if(i == 0) {
            return (u[2] - 2.0 * u[1] + u[0]) / (m_d_rho * m_d_rho);
        }
        if(i == m_n_rho - 1) {
            return (u[m_n_rho - 1] - 2.0 * u[m_n_rho - 2] + u[m_n_rho - 3]) / (m_d_rho * m_d_rho);
        }
    } else {
        double dt = std::log(10.0) / m_n10_rho;
        double rho = m_rho_vals[i];
        ...
        // interions: 4th order finite difference
        if(i >= 2 && i <= m_n_rho - 3) {
            d2u_dt2 = (-u[i + 2] + 16.0 * u[i + 1] - 30.0 * u[i] + 16.0 * u[i - 1] - u[i - 2]) /
                      (12.0 * dt * dt);
        }
        // one point from boundary: 2nd order finite difference
        if(i == 1 || i == m_n_rho - 2) {
        ...
        // boundary: 1st order one sided
        if(i == 0) {
            d2u_dt2 = (u[2] - 2.0 * u[1] + u[0]) / (dt * dt);
        }
        if(i == m_n_rho - 1) {
            d2u_dt2 = (u[m_n_rho - 1] - 2.0 * u[m_n_rho - 2] + u[m_n_rho - 3]) / (dt * dt);
        }
