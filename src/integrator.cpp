...
double compute_error(const std::vector<double>& u1, const std::vector<double>& u2,
    ...
    if(u1.size() != u2.size()) {
        std::cerr << "Arrays don't have the same size. V1.size() = " << u1.size()
                  << ", V2.size() = " << u2.size() << "\n";
    ...
    for(size_t i = 0; i < u1.size(); ++i) {
        ...
        double tolerance =
            absolute_tolerance + relative_tolerance * std::max(std::abs(u1[i]), std::abs(u2[i]));
...
integrate_adaptive(const std::vector<double>& state_init, double t_start, double t_end,
    ...
    while(sign * t < sign * t_end) {
        ...
        if(error < 1.0) {
            ...
            // collect snaphost
            if(snapshots) {
                while(next_snap < snap_targets.size() &&
                      sign * t >= sign * snap_targets[next_snap]) {
        ...
        double factor = (error > 0) ? std::clamp(cfg.safety * std::pow(1.0 / error, 0.25),
                                                 cfg.factor_min, cfg.factor_max)
                                    : cfg.factor_max;
        dt *= factor;

        if(std::abs(dt) < cfg.dt_min) {
            std::cerr << "[ERROR] dt below minimum, aborting\n";
            break;
        }
    }
